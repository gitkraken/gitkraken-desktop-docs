...
description: View a history of the new features and fixes in GitKraken Desktop's Version 9.
...
This release notes page tracks what changed in GitKraken Desktop version 9, including new features, improvements, and bug fixes for each release. Use it when you need historical product changes, want to verify when a version 9 capability shipped, or need to compare old behavior before planning an upgrade.

**Requirements and limits**
- This page is a version 9 release history for older GitKraken Desktop installations.
- Each release entry documents only the behavior and fixes that applied to that specific version.
- Product capabilities described here may differ from current GitKraken Desktop behavior.
- Use current documentation for present workflows and this page for historical version verification.
...
### Bug Fixes 🐛
 - Fetch:
   - Fixed an issue where a remote can't be fetched again until GitKraken Desktop is restarted.
   - Fixed an issue on macOS where GitKraken Desktop might not find the GPG path.
 ...
 - Fixed an issue where, if the user became logged out, GitKraken Desktop wouldn't acknowledge that the user was logged out and wouldn't allow the user to log back in.
...
### New ✨
 ...
 - You can now host Cloud Patches on your own dedicated storage for the highest level of security (Requires an Enterprise plan).
   ...
   - When creating a Cloud Patch, GitKraken Desktop will display a message to confirm it will be securely stored on your organization's storage.
...
### Bug Fixes 🐛
 ...
 - Fixed an issue on macOS where opening a file in Finder from GitKraken Desktop could cause Finder to freeze.
...
### Bug Fixes 🐛
 ...
 - Fixed an issue where some deep links may not be recognized by GitKraken Desktop.
...
- Experimental Feature - Cloud Patches:
  ...
  - Apply a Cloud Patch by following a Cloud Patch URL to open in GitKraken Desktop and then review the contents in the Commit Panel.
...
- GitKraken Desktop will now attempt to pull Date/Time settings from OS preferences, along with an override in <kbd>Preferences > UI Customization</kbd>.
...
### Bug Fixes 🐛
 ...
 - Fixed an issue where GitKraken Desktop couldn't start if the last opened repository was no longer openable.
 ...
 - Fixed an issue where GitKraken Desktop wouldn't show that a merge was active after resolving a conflict.
...
### Improvements 🙌
 - Git Executable:
   - The Git Executable feature will now be enabled by default for all GitKraken Desktop users.
...
### Improvements 🙌
- Git Executable has been enabled for all GitKraken Desktop users to allow Git to directly perform actions like fetch, commit, and much more instead of the Nodegit library. 
...
- Added logging for Git commands with GitKraken Desktop log system.
...
- <kbd>Experimental</kbd> section now available from `Preferences`, and users may now opt-in for:
  ...
  - Experiment 2: Bundled Git Executable
    - GitKraken Desktop will use Git instead of the Nodegit library for actions like fetch and commits, plus deliver some performance improvements. 
...
- Improved WSL 2 support for the Linux version of GitKraken Desktop.
 - Users may now install GitKraken Desktop in a WSL 2 distribution with WSLg and work with their Linux repos.
  - For the best experience, we recommend users also install GitKraken Desktop on their Windows machine to work with repos hosted outside their WSLg distribution. 
...
We know how tedious it can be to figure out what repos have recent WIPs and which repos to open next. That's why we're so excited to present GitKraken Desktop 9.0, and transform your development workflow.
...
- Workspaces ✨
  ...
  - Cloud Workspaces (previously _Personal_ and _Shared_) are enhanced with Pull Requests and Issues from hosting and issue tracking services.
...
- UI / Themes 🎨
  ...
  - Menu bar and context menus will now match the GitKraken Desktop theme in Windows.
