---
title: GitKraken Desktop 7.x
description: View a history of the new features and fixes in GitKraken Desktop's Version 7.
...
This release notes page tracks what changed in GitKraken Desktop version 7, including new features, improvements, and bug fixes from each release. Use it when you need historical version details, want to confirm when an older capability was introduced, or need context before upgrading long-lived installations.

**Requirements and limits**
- This page is a historical version 7 release archive, not a guide to current GitKraken Desktop behavior.
- Each release entry applies only to the version listed in that section.
- Older plan names, integrations, and UI references may not match current product terminology.
- Use this page for upgrade context and historical verification, and use current docs for present-day workflows.
...
_Ahoy there! The crew has been hard at work swabbing the decks and trimming the sails to bring you the improvements in GitKraken Desktop v7.7.2 release. Raise anchor and prepare to update!_ ⛵

### New ✨

- Users who are leveraging Git hooks in their workflows will be happy to learn that GitKraken Desktop now supports the Git configuration [`core.hooksPath`](https://git-scm.com/docs/githooks) variable. Users can also change the directory for where a repository’s hooks are stored from inside GitKraken Desktop by navigating to <kbd>Preferences</kbd> → <kbd>Git Hooks</kbd>.
...
- Untracked files will no longer appear in GitKraken Desktop Teams Panel if those files have not been modified.

### Bug Fixes 🐛

- GitKraken Desktop Merge Conflict Editor users can now scroll horizontally, even if the scrollbar is over a conflicted line.
- When switching tabs, GitKraken Desktop will now remember what panes users had collapsed, as well as which branches & tags had been hidden.
...
- All users will notice improved performance when switching between tabs in GitKraken Desktop.
...
- When opening repositories in GitKraken Desktop, users will no longer get stuck on the loading screen.
- GitKraken Desktop users will no longer encounter a loop repeatedly asking for their SSH passphrase.
...
- Introducing GitKraken Teams:
    - GitKraken Desktop users with Pro and [Enterprise Cloud](https://www.gitkraken.com/gitkraken-enterprise?product=gitkraken&source=help_center) accounts can now create and manage teams within their organization.
...
- Github.com users may now utilize the Pull Request view! Selecting a pull request in the left panel will bring up the pull request view which will allow users to:
    ...
    - Merge a pull request from GitKraken Desktop.
    - Submit reviews by leaving a comment, approving the PR, or requesting changes.

- GitKraken Desktop users at all plan levels can now create and edit issues on GitHub and GitLab public repositories. 

### Improvements 🙌

_We’re giving it all we got..._

- The cherry pick option has been removed from the context menu that appears after selecting multiple commits in the graph. GitKraken Desktop does not support the ability to cherry pick multiple commits...yet.  
...
- The `.gitignore` file generated when initializing a repository form GitKraken Desktop will no longer be set as read-only.
...
### Bug Fixes 🐛
- Resolved an issue that was causing GitKraken Desktop to sporadically crash on Mac and Linux when performing certain actions.
- Fixed another crash that could occur when performing checkout operations.

### Notes
- In an upcoming release of GitKraken Desktop, the minimum supported version of GitHub Enterprise will be 2.20.24 in order to use the integration. We anticipate this change to go live at the beginning of May.
...
- Resolved an issue that was causing GitKraken Desktop to crash for some users when trying to push.
...
- Repositories containing empty annotated tags will no longer cause GitKraken Desktop to crash on Mac and Linux. 
- Squashing in interactive rebase will no longer cause GitKraken Desktop to deadlock. 
...
_We might not wield the same superpowers as Wanda, but it kind of seems like it with these recent updates to the legendary GitKraken Desktop._


### Improvements 🙌

- The tooltip display has improved when dragging-and-dropping branches in the left panel. 
- If a WIP node is present when a user selects <kbd>Reset Local to Here</kbd> to checkout a remote branch, GitKraken Desktop will now create a stash.  
...
<strong>GitKraken:</strong> Ayy here's a hotfix for a big ol oopsie whoopsie we made (aka critical stability improvements for syncing GitKraken Desktop notifications).
...
- A notifications system has been added to GitKraken Desktop to alert users of important messages such as version updates, bug fixes, and more helpful information. These notifications can be accessed via the bell icon 🔔 in the upper right corner, and notification settings can be configured via <kbd><i> <i class="fas fa-cog"></i> Preferences    <i class='fa fa-caret-right'></i>     Notifications</i></kbd>
...
🦃 _Get ready to gobble up these epic GitKraken Desktop improvements and bug fixes._ 

### Improvements 🙌

- After a commit action has been made in GitKraken Desktop, a timestamp showing the duration of the action will be added to the activity log. 

### Bug Fixes 🐛

- The GitLab Self-Managed issue tracking integration will work for instances that have a URL path or protocol. 
- Fetching and auto-fetching while a file is open in GitKraken Desktop will no longer reset the scroll position on the Monaco text editor. 
...
_GitKraken Desktop v7.4.0 release is the key to enjoying an even more productive workflow._ 🗝

### New ✨
_These are the key_ 🔑 _points of GitKraken Desktop v7.4.0_
...
- We have added the option to display [additional columns](/start-here/interface/#columns) of information in the commit graph: 
    ...
    - Commit date/time
    - Commit SHA
- GitHub users now have the ability to create draft pull requests in GitKraken Desktop. 
...
- Users will again receive a notification when GitKraken Desktop automatically resolves a merge conflict. 
...
- Finishing a release branch with Gitflow when the commit on the local master and develop branches are the same will no longer cause GitKraken Desktop to display a blank screen. 
...
- We have resolved an issue causing the Ubuntu snap version of GitKraken Desktop to crash for some users. 
...
_It’s a cephalopod party! GitKraken Desktop v7.3 brings you issue tracking integration for GitHub and GitHub Enterprise - raise your tentacles!_

### New ✨

🐙🐈 _Mona and Keif_ 🦑_have been friends for a long time, and now, they have even more to celebrate in their friendship._

GitKraken Desktop now supports issue tracking integrations for GitHub and GitHub Enterprise! GitHub Issues users now have the ability to perform the following actions from GitKraken Desktop: 
...
_v7.2 of GitKraken Desktop is downright foxy._ 😍 _Introducing GitLab Issue Tracking integration._ 


### New ✨

_GitLab users will feel even more cunning_ 🦊 _while managing issues directly from GitKraken Desktop._ 

- GitKraken Desktop now supports issue tracking integration for GitLab & GitLab Self-Managed! GitLab Issues users now have the ability to perform the following from GitKraken Desktop:
    - View and filter a list of issues from the left panel. 
    - Create a branch tied to an issue. 
    - View issue details. 
    - Edit assignee of an issue.*
...
- GitKraken Desktop will no longer disconnect users who utilize multiple services that reside on the same base domain. 
...
🎶_Trello from the other side! Introducing v7.1 of GitKraken Desktop, now with Trello integration._ 

### New ✨

_Hello? Can you hear us? It’s GitKraken Desktop here to announce Trello integration._ 

- GitKraken Desktop now integrates with Trello as an issue tracker! Once the connection has been authorized, Trello users will be able to accomplish the following from GitKraken Desktop: 
    ...
    - Create new Trello cards from within GitKraken Desktop.* 
...
- For users who have integrated an issue tracker: we have added an option to the context menu for branches tied to an issue or card allowing users to view the issue or card both inside GitKraken Desktop and the respective issue tracker.
...
- GitKraken Desktop is now able to detect all versions of IntelliJ.
...
_GitKraken Desktop has been updated to bring you a slew of new improvements and bug fixes._
...
- When starting a pull request from a branch associated with a Jira or GitKraken Boards issue, GitKraken Desktop will now populate pull request titles.
...
- Default Jira filters that are created inside GitKraken Desktop will no longer throw an error if the project prefix was a JQL reserved word.
...
_Introducing version 7 of GitKraken Desktop: The Best Way to Jira._

<h3>New ✨</h3>

_Got issues? Keep them in Jira! Now you’ll have no issues tracking your issues because you can stay in GitKraken Desktop.  New integrations for Jira Issue Boards and GitKraken Boards make it possible to resolve and manage issues directly from GitKraken Desktop._

<ul>
 <li>GitKraken Desktop now integrates with the following issue trackers:
  <ul>
   <li>Jira (Cloud & Self-Hosted)</li>
   <li>GitKraken Boards</li>
   </ul>
 </li>
 <li>After setting up the integration, users will be able to do the following from GitKraken Desktop:
  <ul>
...
- When creating a GitHub pull request in GitKraken Desktop, users can now search for reviewers and assignees by name or username.
...
- For Windows users: the <kbd>Open with GitKraken</kbd> menu option will now open a new tab inside of GitKraken Desktop instead of a new window.
...
- Users can successfully initialize a repository on a Bitbucket server from GitKraken Desktop.
...
- For Snap users: the snap for GitKraken Desktop has been moved to the classic confinement! Before updating to v7.0.0 of GitKraken Desktop, please install the snap with the `--classic` flag.
