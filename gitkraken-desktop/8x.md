...
description: View a history of the new features and fixes in GitKraken Desktop's Version 8.
...
This release notes page tracks what changed in GitKraken Desktop version 8, including feature releases, UI changes, and bug fixes across each update. Use it when you need historical context for older installations, want to confirm when a version 8 feature shipped, or need release-by-release details before upgrading.

**Requirements and limits**
- This page documents historical version 8 releases rather than current GitKraken Desktop behavior.
- Each update section is tied to the release number and date shown in that entry.
- Some integrations, UI elements, and features described here may no longer exist or may work differently now.
- Use this page to verify version history, and use the current docs for active product guidance.
...
- When a GitLab token expires, GitKraken Desktop will now show a prompt to re-login. This should address previous issues working with GitLab repos.
...
- Fixed an issue where some Linux distributions could no longer launch GitKraken Desktop.
- Fixed an issue where Window Server could no longer launch GitKraken Desktop.
...
- GitKraken Desktop has been updated to Electron 17.
...
The signs are all around us…and they’re pointing to an epic release of GitKraken Desktop v8.8.
...
GitKraken Desktop v8.7 has been released! Whether you're covering a tear, or just adding some flair – we’ve got you patched.

### New + Improved ✨ 

_We hemmed in some new threads._

- GitKraken Desktop now supports the ability to create and apply patches.
...
- Naming branch from issue - When viewing an issue from inside GitKraken Desktop, there is now a short text field for naming the branch when creating a branch from the issue.
- Git LFS performance improvements:
    - Users will see faster performance when cloning LFS repositories with submodules
    - Users will note much faster performance for general GitKraken Desktop actions, e.g. reset, merge 
...
_You don't have to be a an intergalactic Space Ranger to know that when it comes to navigating a repo, fast is good. You'll be seeing performance improvements…performance improvements everywhere, with the release of GitKraken Desktop v8.6._
...
- Repo and Terminal Tab aliases:
    - Users can now set an alias for a repository.
        ...
        - Setting an alias through a Repo Tab will cause GitKraken Desktop to store that name for the repo and reference it as an “Alias Repository”. 
...
- Users can now set GitKraken Desktop to skip submodule updates while performing Git actions, either globally or per repo.
...
_The GitKraken team is proud to announce these improvements to Workspaces, Teams, and the GitKraken CLI. Lower your blast shield, trust your feelings, and prepare for the release of GitKraken Desktop v8.5._
...
- Git LFS Improvements:
    ...
    - Note: Significant work towards reducing checkout times for LFS repos is underway and we plan to include these improvements in the GitKraken Desktop v8.6.0 release, scheduled for June.
- When creating a new Team, members can now be added as part of the creation process.
- Team members are now sorted by username in the Teams section, found in the left panel of GitKraken Desktop.
...
- For Windows users, GitKraken Desktop will now respect the `core.longpaths` setting in `.gitconfig`. Previously, GitKraken Desktop had its own longpaths setting independent of the user’s `.gitconfig` setting.
...
- GitKraken Desktop will now open as expected for users on OpenSSL 3 Linux distributions such as Ubuntu 22.04 and Fedora 36.
...
### Improvements 🙌
_Making GitKraken Desktop a mean, clean Git managing machine._
...
_The improvements to GitKraken Desktop keep Marching forward._
...
### Bug Fixes 🐛
- GitKraken Desktop now supports GitHub’s new GraphQL query types. GitHub users will now see creating, commenting and reviewing pull requests work as expected.
...
_Spring is almost here, but we couldn't wait to bring you these improvements in GitKraken Desktop v8.3.1._
### Improvements 🙌
...
- GitKraken Desktop will remain responsive when adding an [issue tracker integration](https://support.gitkraken.com/start-here/integrations/) supporting a large number of assignable users.
...
_The GitKraken team is going for the gold medal in speed and usability! Sharpen your blades and get ready for the release of GitKraken Desktop v8.3!_
...
GitKraken Desktop v8.3 introduces a new ARM64 compatible version, offering native support for Apple Silicon architectures, as used in Macs with M1 chips.
...
- Fedora 35 users will no longer need to pass the `--no-sandbox` flag to launch GitKraken Desktop.
- GitKraken CLI:
    - New Terminal settings added under <kbd>Preferences</kbd> → <kbd>Terminal</kbd>.  
		- Default Directory - Users can now set the default directory where new Terminal Tabs will open when initiated from the "New CLI Tab" button in the Repo Management Tab or from the Command Palette. 
...
- Themes:
    - Users can customize the commit graph colors in their custom themes.  Examples of `// graph colors` have been added to the default theme files.  Users can refer to  <kbd>Preferences</kbd> → <kbd>UI Customization</kbd> -> <kbd>Theme</kbd> to locate the GitKraken Desktop theme folder on their computer.  
...
_Get ready for a multiverse bending reveal!  We are unleashing the cosmic power of GitKraken Workspaces with the release of GitKraken Desktop v8.2!_
...
GitKraken Workspaces allow you to gather and access all the required repos for your project in a single tab in GitKraken Desktop. The new GitKraken Workspaces tab shows you the status for the last branch that you checked out, how far ahead and/or behind in commits you are, and if there is an active WIP for each included repository. 
...
- Users can view details and trigger the following actions on any or all of the repositories in a workspace at once:
    ...
    - Open multiple repos in GitKraken Desktop, or in an external editor, such as VS Code, IntelliJ, Atom, or Sublime Text.
...
- Users may now create their own custom GitKraken Desktop themes. 
...
- An activity status will now display on the avatars of members in your GitKraken Organization or Team, indicating if they are actively using GitKraken Desktop. This activity status icon is visible in the Team View section in the Left Panel, or in the Organization view in Preferences.
...
- When users open the pull request panel and click on a pull request while the left panel is minimized, GitKraken Desktop will now open the pull request view.
...
- After using GitKraken Desktop to execute a cherry-pick and resolving any arising conflicts, the Git CLI will no longer report that a cherry-pick is currently active.
...
_Get ready to celebrate the release of GitKraken Desktop v8.1.1 with these puns you can tell next time you are hanging out at the ol' foo bar. If you don't get these jokes, `whoami` to judge?_
...
- When working with GitHub and GitLab wiki repos, GitKraken Desktop will no longer throw an error message about being unable to fetch pull requests.
...
_Autumn brings sweaters, pumpkin spice, and apple picking. It is also bringing some awesome updates to GitKraken Desktop as we release v8.1.0!_

### New ✨
_The seasons continue to change and so does GitKraken Desktop...for the better!_

- GitKraken Desktop can now identify weak SSH keys and provide an easy way to remove and replace them. Read more on [the GitKraken blog](https://gitkraken.com/blog/weak-ssh-key-issue-fix).
...
- GitKraken CLI:
    ...
    - When selecting an auto-suggest option with the mouse, GitKraken Desktop will now refocus on the terminal prompt.
...
- Users can now open specific comments on GitHub.com from the comments inside GitKraken Desktop's pull request or issue views.
...
- GitKraken Desktop users can now search for a particular tab and navigate the menu using the keyboard when using the tab dropdown menu.
...
- Gitflow branch folders and tag folders collapse and expand as expected when selected in the left panel in GitKraken Desktop.
- Layout and options have been optimized in the Organization panel in GitKraken Desktop's Preference view.
...
The true power of the Fellowship of developer collaboration is unleashed in GitKraken Desktop's v8.0!
### New ✨
...
The GitKraken CLI adds a Git-enhanced, keyboard-driven, terminal experience to GitKraken Desktop. Get the powerful Git visualizations GitKraken Desktop is known for—like the commit graph—in the context of a Terminal Tab. Plus auto-suggest and auto-complete for Git commands to help you drive (and learn) Git faster, with fewer mistakes.
...
During the GitKraken CLI Preview we are looking for feedback to help improve this feature. Please use the `Feedback` form linked in the bottom right corner of your GitKraken Desktop app, or email us at feedback@gitkraken.com.


- Users can open Terminal Tabs in a number of ways:
    - Clicking the "New Terminal Tab" button after opening a New tab in GitKraken Desktop.
...
- Some of the legendary Git visualizations that GitKraken Desktop is known for are also accessible within Terminal Tabs. To quickly view the commit graph, file diff, history, and blame view, simply type these commands:
...
We've added the ability to share deep links to specific remote repositories, commits, branches, and tags in GitKraken Desktop. This allows users to more easily collaborate and save time when working in issue queues or Git pull requests.  

- When shared, these links will both open and focus GitKraken Desktop to the linked repository. The links will focus GitKraken Desktop on the commit, branch, or tag specified. When specifying a branch, the latest commit of that branch will be focused.
- The links can be found in GitKraken Desktop's context menus under `Copy link to remote/commit/branch/tag`.
<br>
<br>
...
GitKraken Desktop now works with the Git Integration for Jira app to allow quick navigation between the client and Jira when viewing commits and file diffs related to Jira issues.

- In GitKraken Desktop, you’ll find buttons and links to open the following in Git Integration for Jira:
...
- In the Git Integration for Jira, anywhere you view commits and diffs will automatically  have links to open them in GitKraken Desktop.
